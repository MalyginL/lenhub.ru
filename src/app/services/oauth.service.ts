...
export class OauthService {
  ...
  auth(username: string, password: string): Observable<boolean> {
    ...
    return this._http.post<any>(this.url + '/oauth/token', 'grant_type=password&username=' + username + '&password=' + password, {headers: headers}).pipe(map((data:any) => {
  ...
  validate(): Observable<boolean> {
    ...
    return this._http.get(this.url + '/malygin/validate', {headers: headers}).pipe(map(() => {
      this.isAuth = true;
      return true;
    }),catchError(() => {
